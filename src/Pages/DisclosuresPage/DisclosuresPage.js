...
function DisclosuresPage() {
  ...
  const {
    isLoading: AnalystEstimatesLoading,
    data: AnalystEstimatesData,
    refetch: AnalystEstimatesRefetch,
  } = useQuery(
    ["AnalystEstimates"],
    () =>
      axios
  ...
  const { isLoading: eventsLoading, data: eventsData } = useQuery(
    ...
    () =>
      axios
        .get(
          `https://data.argaam.com/api/v1/json/ir-widget/events?recordSize=30`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
